...
export const HouseProvider = (props) => {
  ...
  const addListing = (listingObj) => {
    // debugger
    return fetch(`https://swipehome-d73a6-default-rtdb.firebaseio.com/houses`, {
      method: "POST",
      headers: {
  ...
  const getHouseById = houseId => {
    return fetch(`https://swipehome-d73a6-default-rtdb.firebaseio.com/houses/${houseId}?_expand=user`)
    .then(res => res.json())
  }
  ...
  const getListingsByUserId = userId => {
    return axios.get(`https://swipehome-d73a6-default-rtdb.firebaseio.com/houses?userId=${userId}`)
    .then((res) => res.data)
    .then(data => setListings(data))
  }

  const deleteListingById = id => {
    return axios.delete(`https://swipehome-d73a6-default-rtdb.firebaseio.com/houses/${id}`)
  }
